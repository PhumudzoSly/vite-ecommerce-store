...
export function TrackOrderPage() {
  ...
  const handleTrack = () => {
    ...
    setTimeout(() => {
      ...
      setTrackingStatus(Math.floor(Math.random() * 3)); // 0, 1, or 2
      setLoading(false);
    }, 1500);
  };
      ...
      <div className="container mx-auto px-4 max-w-xl text-center space-y-12">
        ...
        {trackingStatus !== null && (
          <div className="grid grid-cols-3 gap-4 pt-4">
            {[
              { label: "Ordered", icon: Package, done: trackingStatus >= 0 },
              { label: "Shipped", icon: Truck, done: trackingStatus >= 1 },
              {
                label: "Delivered",
                icon: CheckCircle2,
                done: trackingStatus >= 2,
              },
            ].map((step, i) => (
              <div
                key={i}
              ...
              >
                <div
                  className={`h-10 w-10 rounded-full border-2 flex items-center justify-center transition-colors duration-500 ${step.done ? "border-primary bg-primary/10" : "border-muted"}`}
                >
