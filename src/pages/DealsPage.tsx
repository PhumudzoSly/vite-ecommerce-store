    ...
    <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
      <header className="mb-8 space-y-4">
        <div className="rounded-2xl border border-primary/20 bg-primary/5 p-5">
          <div className="flex items-center gap-2 text-primary">
            ...
            <p className="text-sm font-semibold uppercase tracking-wide">Deals of the day</p>
          </div>
          <h1 className="mt-2 text-3xl font-bold tracking-tight text-foreground">Best discounts</h1>
          <p className="mt-2 text-muted-foreground">
            Save up to {bestDiscount}% on products fetched from Fake Store API.
          </p>
        </div>
        ...
        <div className="flex flex-col gap-3 sm:flex-row">
          ...
          <Select
            value={selectedSort}
            onValueChange={(value) => updateQueryParam("sort", value, "discount-desc")}
          >
      ...
      {!dealsQuery.isLoading && dealsQuery.isError && (
        <div className="flex min-h-56 flex-col items-center justify-center rounded-2xl border border-dashed border-destructive/40 px-6 py-10 text-center">
          ...
          <p className="text-sm font-medium text-foreground">Could not load deals</p>
          <p className="mt-1 text-sm text-muted-foreground">
            {dealsQuery.error instanceof Error ? dealsQuery.error.message : "Please try again later."}
          </p>
        </div>
      )}

      {!dealsQuery.isLoading && !dealsQuery.isError && sortedDeals.length === 0 && (
        <div className="flex min-h-56 flex-col items-center justify-center rounded-2xl border border-dashed border-border px-6 py-10 text-center">
          <p className="text-sm font-medium text-foreground">No deals in this category</p>
          <p className="mt-1 text-sm text-muted-foreground">Try another category filter.</p>
        </div>
      )}
      ...
      <ProductDetailsDialog
        product={selectedDeal ? { ...selectedDeal, price: selectedDeal.dealPrice } : null}
